...
if __name__ == "__main__":
    ...
    if vae_type == "vae_dense":
        ...
        vae = VAE_Conv(
            seq_len=T, feat_dim=D, latent_dim=latent_dim, hidden_layer_sizes=[100, 200]
        )
        ...
        vae = TimeVAE(
            ...
            feat_dim=D,
            latent_dim=latent_dim,
            hidden_layer_sizes=[50, 100, 200],  # [80, 200, 250]
    ...
    early_stop_callback = EarlyStopping(
        monitor=early_stop_loss, min_delta=1e-2, patience=30
    )
    reduceLR = ReduceLROnPlateau(monitor="loss", factor=0.5, patience=15)
    ...
    if latent_dim == 2:
        utils.plot_latent_space_timeseries(vae, n=8, figsize=(20, 10))
