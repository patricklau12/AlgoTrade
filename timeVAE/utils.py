...
class MinMaxScaler_Feat_Dim:
    ...
    def __init__(self, scaling_len, input_dim, upper_bound=3.0, lower_bound=-3.0):
    ...
    def fit(self, X, y=None):
        ...
        self.min_vals_per_d = np.expand_dims(
            np.expand_dims(X_f.min(axis=0).min(axis=0), axis=0), axis=0
        )
        self.max_vals_per_d = np.expand_dims(
            np.expand_dims(X_f.max(axis=0).max(axis=0), axis=0), axis=0
        )
    ...
    def transform(self, X, y=None):
        assert (
            X.shape[-1] == self.min_vals_per_d.shape[-1]
        ), "Error: Dimension of array to scale doesn't match fitted array."
